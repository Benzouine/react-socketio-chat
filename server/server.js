...
const colorList = ['#e6194b', '#3cb44b', '#ffe119', '#4363d8', '#f58231', '#911eb4', '#46f0f0', '#f032e6'];
...
io.on('connection', (socket) => {
  ...
  socket.on('set-nickname', (nickname) => {
    ...
    console.log(`Nickname set for socket ${socket.id}: ${nickname} with color ${colors[socket.id]}`);
  });
  ...
  socket.on('chat message', (msg) => {
    ...
    io.emit('chat message', { message: msg, displayName, color: displayColor, timestamp });
  });
